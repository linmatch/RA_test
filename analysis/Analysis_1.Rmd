...
ggplot(df_total_pivot) +
  ...
  geom_line(aes(x = Region, y = percent_of_national_db*100000, group=1), color = "red")+  scale_y_continuous(name = "State Database Total",labels = function(x) format(x, scientific = FALSE), sec.axis = sec_axis(~./100000, labels = function(x) format(x, scientific = FALSE), name = "% of National Database")) +
   geom_text(aes(x = Region, y = state_db_total, label = scales::comma(state_db_total), vjust = -0.5), color = "black", size = 3) +
  geom_text(aes(x = Region, y = percent_of_national_db*100000, label = scales::percent(percent_of_national_db/100), vjust = 1.5), color = "blue", size = 3) +
...
```{r}
ggplot(df_state_pivot2, aes(x = State, y = Database, fill = `Demographic Group`)) +
...
```{r}
ggplot(df_state_pivot2, aes(x = State, y = Population, fill = `Demographic Group`)) +
geom_bar(stat = "identity") +
  labs(title = "Compare composition of Population Representation of each state",
       x = "State", y = "Value(%)",
       fill = "Demographic Group") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
